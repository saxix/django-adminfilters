...
def get_elements(selenium):
    container = selenium.find_element(By.ID, 'adminfilters_filter_save')
    text = container.find_elements(By.CSS_SELECTOR, 'input')
...
def test_initial_status(admin_site):
    __, select, input_text, button, cl = get_elements(admin_site.driver)
    assert not input_text.is_enabled()
